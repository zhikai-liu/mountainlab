...
SSAbstractView::SSAbstractView(QWidget* parent)
    : QWidget(parent)
{
...
void SSAbstractViewUnderlayPainter::paint(QPainter* painter)
{
    ...
    //current location
    if (selected_xmin < 0) {
        Vec2 p0 = q->plot()->coordToPix(vec2(q->currentX(), 0));
        Q_UNUSED(p0);
...
QString make_time_tick_label(double val)
{ //ms
    ...
    if (val < 1000) {
        ...
        if (hold == 0) {
            if (n_hr == 1)
                ...
                return QString("%1 hours").arg(n_hr);
        }
...
int SSAbstractView::getTimepointForX(int x)
{
    if (d->m_timepoint_mapping.totalSize() > 1) {
        if ((0 <= x) && (x < d->m_timepoint_mapping.totalSize())) {
            return d->m_timepoint_mapping.value1(x);
        }
        else
            return -1;
    }
...
{
    if (d->m_current_channel == channel)
        return;
...
void SSAbstractView::setTimepointMapping(const DiskReadMdaOld& X)
{
...
{
    if ((d->m_selected_xmin == range.x) && (d->m_selected_xmax == range.y))
        return;
...
void SSAbstractView::setCurrentTimepoint(int tt)
{
    ...
    if ((d->m_timepoint_mapping.totalSize() > 1) && (x0 >= 0)) {
        for (int i = 0; i < d->m_timepoint_mapping.totalSize(); i++) {
            if (d->m_timepoint_mapping.value1(i) == x0) {
                x0 = i;
                break;
            }
        }
...
void SSAbstractViewPrivate::do_zoom(double center_x, double frac)
{
    ...
    if ((x_right - x_left <= 10) && (frac < 1))
        return; //don't zoom too close
    q->setXRange(vec2(x_left, x_right));
}
...
void SSAbstractViewPrivate::scroll_to_current_x_if_needed(bool force_center)
{
    ...
    if (XR.y > m_max_timepoint) {
        XR.x += m_max_timepoint - XR.y;
        XR.y += m_max_timepoint - XR.y;
    }
    q->setXRange(XR);
}
...
void SSAbstractViewPrivate::on_left_right_arrow(bool left)
{
    ...
    if (range.x + dx < 0)
        dx = -range.x;
    if (range.y + dx > m_max_timepoint)
        dx = m_max_timepoint - range.y;
    range.x += dx;
    range.y += dx;
