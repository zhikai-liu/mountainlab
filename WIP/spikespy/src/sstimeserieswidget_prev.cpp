...
SSTimeSeriesWidget::SSTimeSeriesWidget(QWidget* parent)
    : QWidget(parent)
{
...
void SSTimeSeriesWidget::addView(SSAbstractView* V)
{
    ...
    if (d->m_views.isEmpty())
        V->setTimelineVisible(true);
    else
        V->setTimelineVisible(false);
...
void SSTimeSeriesWidget::slot_extract_clips()
{
    ...
    if (dlg->param("save_to_disk").toBool()) {
        ...
        if (save_to_path.isEmpty())
            return;
        if (QFileInfo(save_to_path).suffix() != "mda")
            save_to_path += ".mda";
    }
    ...
    MemoryMda TL0;
    if (TL)
        TL0 = TL->getTimepointsLabels(0, TL->getMaxTimepoint());
    else
        qWarning() << "TL is null in slot_extract_clips";
    ...
    QStringList args;
    args << data_path << TL_path << out_path << QString("--clipsize=%1").arg(clipsize) << "--labels=" + labels_txt;
    if (fixed_clipsize)
        args << "--fixed-clipsize";
...
void SSTimeSeriesWidget::slot_extract_comparison_clips()
{
    ...
    QStringList args;
    args << data_path1 << data_path2 << TL1_path << TL1_path << out_path1 << out_path2 << QString("--clipsize=%1").arg(clipsize) << "--labels=" + labels_txt;
