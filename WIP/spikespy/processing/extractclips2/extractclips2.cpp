    ...
    const QMap<QString, QVariant>& params)
{
...

    {
        ...
        if (!mda_read_header(&HH_TL1, infile_TL1))
            return 0;
        if (HH_TL1.dims[0] != 2) {
        ...
        for (int i = 0; i < N2; i++) {
            ...
            if (l0 == label1)
                label1_times.insert(t0);
        }
        free(buffer_TL1);
    }
    {
        struct MDAIO_HEADER HH_TL2;
        if (!mda_read_header(&HH_TL2, infile_TL2))
            return 0;
        if (HH_TL2.dims[0] != 2) {
            printf("Unexpected dimensions for TL1\n");
            return 0;
    ...
    if (!mda_read_header(&HH_infile1, infile1))
        return 0;
    ...
    if (!mda_read_header(&HH_infile2, infile2))
        return 0;
