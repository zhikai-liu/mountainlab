...
QVector<double> compute_peaks_v2c(ClipsGroup clips, int ch);
QVector<int> consolidate_labels_v2c(DiskReadMda32& X, const QVector<double>& times, const QVector<int>& labels, int ch, int clip_size, int detect_interval, double consolidation_factor);
QList<int> get_sort_indices_c(const QVector<int>& channels, const QVector<double>& template_peaks);
...
ClipsGroup grab_clips_subset_c(ClipsGroup clips, const QVector<int>& inds);
...
#pragma omp critical
        ...
        for (int k = 1; k <= K0; k++) {
            ...
            for (int bb = 0; bb < inds_k.count(); bb++) {
...
#pragma omp critical
        {
            ...
            printf("Channel %d: total of %d clusters.\n", m + 1, K0);
        }
    }

    int num_clusters_safely_included = 0;
    int num_clusters_safely_excluded = 0;
    int num_clusters_pretty_safely_excluded = 0;
    int num_clusters_pretty_safely_included = 0;
    ...
    QList<int> sort_inds = get_sort_indices(times00);
    Mda firings(3, sort_inds.count());
    for (int jj = 0; jj < sort_inds.count(); jj++) {
    ...
    int K;
    {
        ...
        QList<int> sort_inds = get_sort_indices_c(channels, template_peaks);
        QList<int> label_map;
        for (int k = 0; k <= K; k++)
            label_map << 0;
...
QVector<double> compute_abs_peaks_v2c(ClipsGroup clips, int ch)
{
    int T = clips.clips->N2();
    int L = clips.inds.count();
    int t0 = (T + 1) / 2 - 1;
...
QVector<int> find_peaks_below_threshold_v2c(QVector<double>& peaks, double threshold)
{
    QVector<int> ret;
...
QVector<int> find_peaks_above_threshold_v2c(QVector<double>& peaks, double threshold)
{
    QVector<int> ret;
    for (int i = 0; i < peaks.count(); i++) {
        if (peaks[i] >= threshold)
...
QVector<int> do_branch_cluster_v2c(ClipsGroup clips, const Branch_Cluster_V2_Opts& opts, int channel_for_display)
{
    ...
    if (K0 > 1) {
        ...
        while (true) {
            QVector<int> inds_below = find_peaks_below_threshold_v2c(abs_peaks, abs_peak_threshold);
        ...
        if (abs_peak_threshold > max_abs_peak) {
            ...
            QVector<int> inds_below = find_peaks_below_threshold_v2c(abs_peaks, abs_peak_threshold);
            QVector<int> inds_above = find_peaks_above_threshold_v2c(abs_peaks, abs_peak_threshold);
            ...
            if (K_above <= 1) {
                ...
                QVector<int> inds_next_shell = find_peaks_below_threshold_v2c(abs_peaks_above, abs_peak_threshold + opts.shell_increment);
