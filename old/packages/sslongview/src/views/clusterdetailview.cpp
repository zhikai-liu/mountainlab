...
void ClusterDetailView::setClustersToView(const QList<int>& list)
{
    d->m_clusters_to_view = list;
}
...
void ClusterDetailView::prepareCalculation()
{
    ...
    d->m_calculator.timeseries = c->timeseries();
    d->m_calculator.firings = c->firings();
    d->m_calculator.clip_size = c->option("clip_size", 100).toInt();
    d->m_calculator.clusters_to_view = d->m_clusters_to_view;
    update();
}

void ClusterDetailView::runCalculation()
{
    if (d->m_calculator.clusters_to_view.isEmpty()) {
        TaskProgress task;
        task.error() << "No clusters selected in cluster detail view";
        return;
    }
...
void ClusterDetailViewPrivate::compute_total_time()
{
    ...
    m_total_time_sec = c->timeseries().N2() / c->sampleRate();
}
...
DiskReadMda32 mp_compute_templates(const QString& timeseries, const QString& firings, int clip_size, const QList<int>& clusters)
{
    ...
    QString processor_name = "mountainsort.compute_templates";
    ...
    DiskReadMda32 ret(templates_fname);
    //ret.setRemoteDataType("float32");
    //ret.setRemoteDataType("float32_q8"); //to save download time!
    return ret;
}

void mp_compute_templates_stdevs(DiskReadMda32& templates_out, DiskReadMda32& stdevs_out, const QString& timeseries, const QString& firings, int clip_size)
{
...
void ClusterDetailViewCalculator::compute()
{
    ...
    for (int i = 0; i < clusters_to_view.count(); i++) {
    ...
    */
    ...
    for (int aa = 0; aa < clusters_to_view.count(); aa++) {
        task.setProgress((aa + 1) * 1.0 / clusters_to_view.count());
        if (MLUtil::threadInterruptRequested()) {
            task.error("Halted ***");
            return;
        }
