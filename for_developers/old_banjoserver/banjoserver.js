...
function banjoserver(req,onclose,callback,hopts) {
	var action=req.a||'';
	if (!action) {
...
exports.banjoserver=banjoserver;
	...
	this.handle_request=function(REQ,RESP) {
		...
		if (REQ.method == 'OPTIONS') {
			...
			var onclose0=function(to_run_on_close) {
				REQ.on('close',to_run_on_close);
			}
			banjoserver(query,onclose0,function(resp) {
			...
			receive_json_post(REQ,function(obj) {
				...
				banjoserver(obj,onclose0,function(resp) {
					if (resp) send_json_response(resp);
				},hopts);
			});
		}
