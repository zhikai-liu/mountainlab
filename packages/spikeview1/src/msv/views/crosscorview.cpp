...
public:
    ...
    QList<CrosscorViewPanel*> m_panels;
    //int m_num_rows = 1;
...
void CrosscorView::zoomAllTheWayOut()
{
...
void CrosscorView::slot_panel_clicked(int index, Qt::KeyboardModifiers modifiers)
{
    ...
    if (modifiers & Qt::ShiftModifier) {
        ...
        if ((j1 >= 0) && (j2 >= 0)) {
            ...
            for (int j = j1; j <= j2; j++) {
                CrosscorViewPanel* panel = d->m_panels.value(j);
                if (panel) {
        ...
        CrosscorViewPanel* panel = d->m_panels.value(index);
        int indx = panel->property("cluster_data_index").toInt();
...
void CrosscorView::slot_vertical_zoom_in()
{
    d->m_vscale_factor *= 1.1;
    d->update_scale_factors();
}

void CrosscorView::slot_vertical_zoom_out()
{
    d->m_vscale_factor /= 1.1;
    d->update_scale_factors();
}
...
/*
void CrosscorView::slot_set_num_rows(int num_rows)
{
...
void CrosscorViewCalculator::mv_compute_templates(DiskReadMda32& templates_out, const QString& mlproxy_url, const QString& timeseries, const QString& firings, int clip_size, bigint max_events_per_template, FiltOpts filtopts)
{
...
void CrosscorViewPrivate::compute_total_time()
{
...
double CrosscorViewPrivate::get_disksize_for_firing_rate(double firing_rate)
{
...
void CrosscorViewPrivate::update_panels()
{
    ...
    for (int i = 0; i < m_cluster_data.count(); i++) {
        ...
        if (c->clusterIsVisible(CD.k)) {
            CrosscorViewPanel* panel = new CrosscorViewPanel;
...
void CrosscorViewPrivate::update_scale_factors()
{
