...
public:
    EventMarkerLayer(SSLVTimeSeriesViewBase* q0, SSLVTimeSeriesViewBasePrivate* d0)
    {
        q = q0;
        d = d0;
    }
...
public:
    CursorLayer(SSLVTimeSeriesViewBase* q0, SSLVTimeSeriesViewBasePrivate* d0)
    {
        q = q0;
        d = d0;
    }
...
public:
    TimeAxisLayer(SSLVTimeSeriesViewBase* q0, SSLVTimeSeriesViewBasePrivate* d0)
    {
        q = q0;
        d = d0;
    }
...
public:
    StatusLayer(SSLVTimeSeriesViewBase* q0, SSLVTimeSeriesViewBasePrivate* d0)
    {
        q = q0;
        d = d0;
    }
...
public:
    TimeSeriesContentLayer(SSLVTimeSeriesViewBase* q0, SSLVTimeSeriesViewBasePrivate* d0)
    {
        q = q0;
        d = d0;
    }
...
SSLVTimeSeriesViewBase::SSLVTimeSeriesViewBase(MVAbstractContext* context)
    : MVAbstractView(context)
{
    d = new SSLVTimeSeriesViewBasePrivate;
    d->q = this;
...
SSLVTimeSeriesViewBase::~SSLVTimeSeriesViewBase()
{
    this->stopCalculation();
    delete d;
}

void SSLVTimeSeriesViewBase::prepareCalculation()
{
    /*
...
void SSLVTimeSeriesViewBase::runCalculation()
{
    //d->m_calculator.compute();
}

void SSLVTimeSeriesViewBase::onCalculationFinished()
{
...
/*
void SSLVTimeSeriesViewBase::setTimesLabels(const QVector<double>& times, const QVector<int>& labels)
{
    d->m_times = times;
    d->m_labels = labels;
    update();
...
void SSLVTimeSeriesViewBase::setNumTimepoints(long N)
{
    d->m_num_timepoints = N;
}

mvtsv_prefs SSLVTimeSeriesViewBase::prefs() const
{
    return d->m_prefs;
}

void SSLVTimeSeriesViewBase::setPrefs(mvtsv_prefs prefs)
{
    d->m_prefs = prefs;
}

void SSLVTimeSeriesViewBase::setLabelsToView(const QSet<int>& labels)
{
    d->m_labels_to_view = labels;
    this->recalculate();
}

QRectF SSLVTimeSeriesViewBase::contentGeometry()
{
    return d->content_geometry();
}

double SSLVTimeSeriesViewBase::time2xpix(double t) const
{
    return d->time2xpix(t);
}

double SSLVTimeSeriesViewBase::xpix2time(double x) const
{
    return xpix2time(x);
}

void SSLVTimeSeriesViewBase::resizeEvent(QResizeEvent* evt)
{
    d->m_paint_layer_stack.setWindowSize(this->size());
    QWidget::resizeEvent(evt);
}

void SSLVTimeSeriesViewBase::setTimeRange(MVRange range)
{
    SSLVContext* c = qobject_cast<SSLVContext*>(mvContext());
    Q_ASSERT(c);
...
MVRange SSLVTimeSeriesViewBase::timeRange()
{
    SSLVContext* c = qobject_cast<SSLVContext*>(mvContext());
    Q_ASSERT(c);
...
void SSLVTimeSeriesViewBase::setSelectedTimeRange(MVRange range)
{
...
void SSLVTimeSeriesViewBase::setActivated(bool val)
{
...
void SSLVTimeSeriesViewBase::setMarkersVisible(bool val)
{
...
void SSLVTimeSeriesViewBase::setMargins(double mleft, double mright, double mtop, double mbottom)
{
...
void SSLVTimeSeriesViewBase::setClipSize(int clip_size)
{
    d->m_clip_size = clip_size;
    update();
}

void SSLVTimeSeriesViewBase::paintEvent(QPaintEvent* evt)
{
    Q_UNUSED(evt)
...
void SSLVTimeSeriesViewBase::mousePressEvent(QMouseEvent* evt)
{
...
void SSLVTimeSeriesViewBase::mouseReleaseEvent(QMouseEvent* evt)
{
    SSLVContext* c = qobject_cast<SSLVContext*>(mvContext());
    Q_ASSERT(c);
...
void SSLVTimeSeriesViewBase::mouseMoveEvent(QMouseEvent* evt)
{
    if (!d->m_left_click_dragging) {
...
void SSLVTimeSeriesViewBase::wheelEvent(QWheelEvent* evt)
{
    SSLVContext* c = qobject_cast<SSLVContext*>(mvContext());
    Q_ASSERT(c);
...
void SSLVTimeSeriesViewBase::keyPressEvent(QKeyEvent* evt)
{
    SSLVContext* c = qobject_cast<SSLVContext*>(mvContext());
    Q_ASSERT(c);
...
void SSLVTimeSeriesViewBase::slot_scroll_to_current_timepoint()
{
    d->scroll_to_current_timepoint();
}

void SSLVTimeSeriesViewBase::slot_zoom_in()
{
    SSLVContext* c = qobject_cast<SSLVContext*>(mvContext());
    Q_ASSERT(c);
...
void SSLVTimeSeriesViewBase::slot_zoom_out()
{
    SSLVContext* c = qobject_cast<SSLVContext*>(mvContext());
    Q_ASSERT(c);
...
double SSLVTimeSeriesViewBasePrivate::time2xpix(double t)
{
    SSLVContext* c = qobject_cast<SSLVContext*>(q->mvContext());
    Q_ASSERT(c);
...
double SSLVTimeSeriesViewBasePrivate::xpix2time(double xpix)
{
    SSLVContext* c = qobject_cast<SSLVContext*>(q->mvContext());
    Q_ASSERT(c);
...
QRectF SSLVTimeSeriesViewBasePrivate::content_geometry()
{
...
void SSLVTimeSeriesViewBasePrivate::zoom_out(double about_time, double frac)
{
...
void SSLVTimeSeriesViewBasePrivate::zoom_in(double about_time, double frac)
{
    zoom_out(about_time, 1 / frac);
}

void SSLVTimeSeriesViewBasePrivate::scroll_to_current_timepoint()
{
    SSLVContext* c = qobject_cast<SSLVContext*>(q->mvContext());
    Q_ASSERT(c);
...
QString SSLVTimeSeriesViewBasePrivate::format_time(double tp)
{
    SSLVContext* c = qobject_cast<SSLVContext*>(q->mvContext());
    Q_ASSERT(c);
...
void SSLVTimeSeriesViewBasePrivate::update_cursor()
{
...
void StatusLayer::paint(QPainter* painter)
{
    ...
    if (d->m_prefs.show_current_timepoint) {
        double samplerate = c->sampleRate();
        if (samplerate) {
            str = QString("%1 (tp: %2)").arg(d->format_time(c->currentTimepoint())).arg((long)c->currentTimepoint());
        }
        else {
            str = QString("Sample rate is null (tp: %2)").arg((long)c->currentTimepoint());
        }
    }
