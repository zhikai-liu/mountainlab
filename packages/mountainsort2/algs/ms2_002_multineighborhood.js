...
var basic_sort=require('./ms2_002.js');
var os=require('os');

exports.spec=function() {
	var spec0=basic_sort.spec();
	spec0.name='mountainsort.ms2_002_multineighborhood';
	spec0.version=spec0.version+'-0.1';
	spec0.inputs.push({name:'geom',optional:true});
	spec0.parameters.push({name:'adjacency_radius'});
	spec0.parameters.push({name:'consolidate_clusters',optional:true,default_value:'true'});
		...
		function add_neighborhood_step(ch) {
			nbhd_steps.push(function(cb) {
				...
				all_nbhd_firings.push(firings0);
				{
					...
					opts2.consolidate_clusters=opts.consolidate_clusters;
					opts2.consolidation_factor=opts.consolidation_factor;
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ //
	...
	function set_geom_coords() {
		if (opts.geom) {
			info.geom_coords=read_geom_coords(opts.geom);
		}
		else {
			info.geom_coords=[];
			for (var m=1; m<=info.M; m++) {
				info.geom_coords.push([0,0]);
			}
		}
	}
