...
bool p_link_segments(QString firings_path, QString firings_prev_path, QString Kmax_prev_path, QString firings_out_path, QString Kmax_out_path, double t1, double t2, double t1_prev, double t2_prev) {
    ...
    if (!Kmax_out.write64(Kmax_out_path)) return false;
...
namespace P_link_segments {
    ...
    bool should_link(const Mda &CM,const QVector<long> &counts1,const QVector<long> &counts2,int i1,int i2,double link_threshold) {
        if (CM.value(i1,i2)<link_threshold*counts1[i1]) return false;
        if (CM.value(i1,i2)<link_threshold*counts2[i2]) return false;
        return true;
    }
    ...
    Mda get_loose_confusion_matrix(const QVector<double> &times1,const QVector<long> &labels1,const QVector<double> &times2,const QVector<long> &labels2,long K1,long K2,long time_offset_tolerance) {
        ...
        while ((i1<times1_sorted.count())&&(i2<times2_sorted.count())) {
            ...
            if (t2<t1-time_offset_tolerance) i2++;
            else if (t2>t1+time_offset_tolerance) {
                i1++;
                if (i1<times1_sorted.count()) {
                    while ((i2>0)&&(times2_sorted[i2]>=times1_sorted[i1])) i2--;
                }
            }
            else {
