...
function detect_events(timeseries,event_times_out,callback) {
	var detect_interval=Math.ceil(params.detect_interval_msec/1000*params.samplerate);
...
function extract_clips(timeseries,event_times,clips_out,callback) {
	var clip_size=Math.ceil(params.clip_size_msec/1000*params.samplerate);
...
function mp_exec_process(processor_name,inputs,outputs,params,callback) {
	...
	make_system_call(exe,args,{show_stdout:false,show_stderr:false,num_tries:2},function(aa) {
		if (aa.return_code!=0) {
