    ...
    for (int current_component = 0; current_component < num_components; current_component++) {
        ...
        for (int it = 0; it < d->m_num_iterations; it++) {
            ...
            for (int vnum = 0; vnum < num_vectors; vnum++) {
                ...
                if (d->m_weights.width() <= 1)
                    ip = inner_product(N, &working_vectors[vnum * N], component_vector);
                else
                    ip = inner_product_with_weights(N, &working_vectors[vnum * N], component_vector, d->m_weights.ptrX(0));
                for (int j = 0; j < N; j++) {
        ...
        for (int vnum = 0; vnum < num_vectors; vnum++) {
            ...
            if (d->m_weights.width() <= 1)
                ...
                ip0 = inner_product_with_weights(N, &working_vectors[vnum * N], &working_components[current_component * N], d->m_weights.ptrX(0));
        ...
        for (int vnum = 0; vnum < num_vectors; vnum++) {
            if (d->m_weights.width() <= 1)
                subtract_component(N, &working_vectors[N * vnum], &working_components[N * current_component]);
            else
                subtract_component_with_weights(N, &working_vectors[N * vnum], &working_components[N * current_component], d->m_weights.ptrX(0));
        }
    }
