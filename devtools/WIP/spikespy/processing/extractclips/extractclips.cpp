...
int extractclips(FILE* infile, FILE* infile_TL, FILE* outfile, FILE* outfile_TL, FILE* outfile_TM, const QMap<QString, QVariant>& params)
{
    ...
    for (int jj = 0; jj < labels.count(); jj++) {
        int l0 = labels[jj];
        if (!target_label_counts.contains(l0))
            target_label_counts[l0] = 0;
    ...
    if (!mda_read_header(&HH_TL, infile_TL))
        return 0;
    if (HH_TL.dims[0] != 2) {
    ...
    foreach (int t, TL1) {
        ...
        if (!all_labels) {
            ...
            for (int tt = t1; tt <= t2; tt++) {
                foreach (int ll, time_labels_keys)
                    if (time_labels[ll].contains(tt))
                        counts[ll]++;
            }

            foreach (int ll, time_labels_keys) {
                if (target_label_counts.value(ll, 0) > counts.value(ll, 0))
                    okay = false;
            }
        }
        if (okay)
            critical_times.insert(t);
    }
