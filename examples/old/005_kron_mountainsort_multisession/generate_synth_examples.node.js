...
mkdir_safe(dspath0);
var session='';
if ('session' in CLP.namedParameters) {
	session=CLP.namedParameters['session'];
	dspath1=dspath0+'/'+session;
}
else dspath1=dspath0;
mkdir_safe(dspath1);
...
args+=' --waveforms='+bigpath0+'/waveforms-'+session+'.mda';
args+=' --timeseries='+bigpath0+'/raw-'+session+'.mda';
args+=' --firings_true='+bigpath0+'/firings_true-'+session+'.mda';
...
make_system_call(cmd,args,function() {
	make_prv_file(bigpath0+'/waveforms-'+session+'.mda',dspath1+'/waveforms.mda.prv',function() {});
	make_prv_file(bigpath0+'/raw-'+session+'.mda',dspath1+'/raw.mda.prv',function() {});
	make_prv_file(bigpath0+'/firings_true-'+session+'.mda',dspath1+'/firings_true.mda.prv',function() {});
});
